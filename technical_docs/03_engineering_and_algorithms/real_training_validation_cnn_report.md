...
> [!WARNING]
> **CPU-ONLY RUN (`cuda_available: false`).** This report was generated on a CPU PyTorch install. The model is a **2-layer synthetic CNN** (`custom/SimpleCNN-2Layer`), not ResNet-50. Step timing comparisons on CPU have no GPU relevance. Do not cite these numbers as GPU evidence.
...
|---|---|---|---|
| **Step Throughput Time** | `13.9 ms` | `11.1 ms` | **+19.8% Speedup** |
| **Total GPU Hours Needed** | `3.23 hrs` | `2.59 hrs` | **0.64 GPU-Hours Saved** |
| **Total Compute Cost ($)** | `$11.30` | `$9.07` | **$2.23 Gross Savings** |
| **Net Client Dollar Savings** | - | - | **$1.68 Net Saved** |
| **Performance Fee (25%)** | - | - | **$0.56 Earned** |

> [!NOTE]
> Performance Fee is calculated strictly as 25% of verified GPU compute savings ($2.23). Zero cost is incurred if no efficiency gain is achieved.
...
- **DataLoader I/O Stall Percentage:** `84.3%`
...
- **Confidence Score:** `99.0%` | **Risk Level:** `LOW`
- **Auto-apply Safe:** `No (Requires Manual Review)` | **Rollback Mode:** `AUTOMATIC`
- **Evidence:** DataLoader GPU I/O stall measured at 84.3% with num_workers=0.
...
- **Description:** DataLoader is currently causing a 84.3% I/O stall on the GPU because num_workers=0 and pin_memory=False.
- **Actionable Fix:**
```python
