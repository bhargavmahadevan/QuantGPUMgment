...
|---|---|---|---|
| **Step Throughput Time** | `1.6 ms` | `0.9 ms` | **+45.0% Speedup** |
| **Total GPU Hours Needed** | `0.36 hrs` | `0.20 hrs` | **0.16 GPU-Hours Saved** |
| **Total Compute Cost ($)** | `$1.35` | `$0.74` | **$0.61 Gross Savings** |
| **Net Client Dollar Savings** | - | - | **$0.46 Net Saved** |
| **Performance Fee (25%)** | - | - | **$0.15 Earned** |

> [!NOTE]
> Performance Fee is calculated strictly as 25% of verified GPU compute savings ($0.61). Zero cost is incurred if no efficiency gain is achieved.
...
- **DataLoader I/O Stall Percentage:** `100.0%`
...
- **Auto-apply Safe:** `No (Requires Manual Review)`
- **Description:** DataLoader is currently causing a 100.0% I/O stall on the GPU because num_workers=0 and pin_memory=False.
- **Actionable Fix:**
```python
